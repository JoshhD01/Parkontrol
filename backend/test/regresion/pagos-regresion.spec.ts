...
describe('PagosService Regression Tests', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('crear - Regression', () => {
    ...
    it('should throw BadRequestException if pago already exists', async () => {
      const { service, reservasService, pagoRepository } = await createTestingModule();

      const dto = { idReserva: 1, idMetodoPago: 1 };
      const reserva = { id: 1, estado: 'ABIERTA' };

      reservasService.findReservaById.resolves(reserva);
      reservasService.finalizarReserva.resolves({ ...reserva, fechaSalida: new Date() });
      pagoRepository.findOne.resolves({ id: 1 });

      await expect(service.crear(dto as any)).rejects.toThrow(BadRequestException);
    });
  });

  describe('findByReserva - Regression', () => {
  ...
  describe('findPagoById - Regression', () => {
  ...
  describe('findByCliente - Regression', () => {
    it('should return pagos for cliente', async () => {
      const { service, pagoRepository } = await createTestingModule();

      const pagos = [{ id: 1 }];
      pagoRepository.createQueryBuilder.returns({
